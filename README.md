...
> **极简 · 零依赖 · 目录即路由 · 性能优先**
>
> 用 Laravel 1% 的代码量，覆盖 96% 的主流框架功能。
>
...
- **链式 DB** — 查询 / 关联 / 缓存 / 事务 / 软删除 / 自动时间戳 / 多数据库
...
- **本地配置** — `config.local.php` + `.env` 环境变量，不提交 Git
- **PHPUnit** — 内置 Unit/Feature 套件
- **安全工具** — Auth（密码/Session/JWT）· Encryption（AES-256）· Cookie（安全属性+加密）· RateLimiter（限流）
- **网络工具** — Http 客户端 · Redis 封装 · Response 响应 · Pagination 分页器
- **字符串工具** — Str（slug/uuid/mask/camel/snake 等 18 个方法）
...
|------|------:|--------:|---------:|------------:|-----:|
| 框架代码量 | **~3,500 行** | ~400,000 行 | ~150,000 行 | ~80,000 行 | ~5,000 行 |
...
│
├── lib/                   # 框架核心 23 个组件（无需修改）
│   ├── Bootstrap.php      # 框架启动引导
│   ├── Router.php         # 路由解析与分发
│   ├── Core.php           # 基类控制器（所有特性入口）
│   ├── DB.php             # PDO 链式查询（MySQL/PostgreSQL/SQLite）
│   ├── Request.php        # 请求封装
│   ├── Response.php       # 响应封装（JSON/下载/重定向）
│   ├── Validator.php      # 表单验证器（13 种规则）
│   ├── Auth.php           # 密码哈希 + Session + JWT
│   ├── Encryption.php     # AES-256-CBC 加解密
│   ├── Cookie.php         # 安全 Cookie
│   ├── Redis.php          # Redis 完整封装
│   ├── Cache.php          # 多驱动缓存（file/redis/memcache）
│   ├── Http.php           # HTTP 客户端
│   ├── Str.php            # 字符串工具（18 个方法）
│   ├── Env.php            # .env 环境变量加载器
│   ├── Pagination.php     # 独立分页器
│   ├── RateLimiter.php    # 接口限流器
│   ├── Event.php          # 事件总线
│   ├── Queue.php          # 异步队列（database/redis）
...
以下组件在 v1.1 中新增，全部零外部依赖：

| 组件 | 说明 | 示例 |
|------|------|------|
| **Response** | 统一响应封装 | `$this->response->status(201)->json($data)` |
| **Auth** | 密码哈希 + Session + JWT | `Auth::hashPassword()` / `Auth::login()` / `Auth::jwtEncode()` |
| **Encryption** | AES-256-CBC + HMAC 防篡改 | `$enc->encrypt($data)` / `$enc->decrypt($cipher)` |
| **Cookie** | 安全 Cookie（HttpOnly/SameSite/加密） | `$cookie->setEncrypted('token', $val)` |
| **Redis** | 完整封装（7 种数据结构+锁+管道+发布订阅） | `$this->redis->set()` / `$this->redis->lock()` |
| **Http** | cURL HTTP 客户端 | `(new Http)->withToken($t)->post($url, $data)->json()` |
| **Str** | 18 个字符串工具方法 | `Str::slug()` / `Str::uuid()` / `Str::mask()` |
| **Env** | .env 环境变量加载器 | `Env::get('DB_HOST', 'localhost')` |
| **Pagination** | 独立分页器 | `Pagination::fromQuery($query, $page, 10)` |
| **RateLimiter** | 滑动窗口限流（Redis/文件双后端） | `$limiter->tooMany('api:'.$ip, 60, 60)` |

控制器中通过懒加载访问：`$this->db` / `$this->request` / `$this->response` / `$this->redis`
...
[H2CMS](https://github.com/tang30000/h2cms) 示例项目提供 **30 课**完整教程：
